...
int DirectX::LoadImageDataFromFile(BYTE** imageData, D3D12_RESOURCE_DESC& textureDesc, LPCWSTR filename, int &bytesPerRow)
{
	...
	int bitsPerPixel = DirectX::GetDXGIFormatBitsPerPixel(dxgiFormat);
...
HRESULT DirectX::CreateImageDataTextureFromFile(
	ID3D12Device* device, 
	...
	Microsoft::WRL::ComPtr<ID3D12Resource>& textureUploadHeap)
{
	...
	// この関数は、gpuにテクスチャをアップロードするためにアップロードバッファが必要とするサイズを取得
	// 最後の行を除いて、各行は256バイト整列でなければならず、行のバイト単位のサイズにすることができる
	// textureUploadBufferSize =（（（width * numBytesPerPixel）+ 255）＆〜255）*（height-1））+（width * numBytesPerPixel）;
